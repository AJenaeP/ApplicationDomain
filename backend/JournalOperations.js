...
async function getJournal(data) {
  //getting a certain journal
  try {
    let pool = await sql.connect(config);
    let journal = await pool
      .request()
      .input("ref", data.ref)
